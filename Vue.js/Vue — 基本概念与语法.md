...
---
- #### Vue 的数据流是单向的
  ##### 1. 由于 Vue 中数据流是单向的，所以不修改通过 props 传入组件的数据（只针对基本类型，引用类型的本质内存地址，所以不适用于本条规则）是一种最佳实践，不然即使不报错，也会有 warning。
  ##### 2. 由于 Vue 中数据流是单向的，所以 v-model 实际上是一个语法糖，它结合了 v-bind 和 v-on:change 这两个语法。
